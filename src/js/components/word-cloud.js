...
export default class WordCloud {
  constructor({ data, container, width, height, city, colors }) {
    ...
    this.file = `../data/${city}_review_words.csv`;
    this.data = data;
    this.colors = colors;
  ...
  init() {
    ...
    this.chart = cloud().size([this.width - 50, this.height - 50])
      .words(this.frequency_list)
      .rotate(0)
      .fontSize(d => d.size)
